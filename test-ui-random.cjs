...
(async()=>{
 ...
 await run('paid fourth: cancel atomic, accept exactly once, token remains spent',async p=>{
  await p.fill('#score-0','12');await p.selectOption('#roll','3');await p.check('#lock-0');const before=await snapshot(p);let dialogs=0;
  ...
  await fixed(p,[0]);p.once('dialog',async d=>{dialogs++;await d.accept();});await p.click('#random-remaining');await expect(p.locator('#roll')).toHaveValue('4');await expect(p.locator('#score-total')).toHaveText('2');await expect(p.locator('#score-0')).toHaveValue('12');await expect(p.locator('#extra')).not.toBeChecked();await expect(p.locator('#random-remaining')).toBeDisabled();assert.deepEqual(await dice(p),['2','1','1','1','1']);
  await p.locator('#random-remaining').evaluate(b=>{b.disabled=false;b.click();});await expect(p.locator('#score-total')).toHaveText('2');assert.equal(dialogs,2);await p.click('#random-all');await expect(p.locator('#extra')).not.toBeChecked();await expect(p.locator('#score-total')).toHaveText('2');await p.selectOption('#roll','3');await expect(p.locator('#random-remaining')).toBeDisabled();await p.fill('#score-0','18');await expect(p.locator('#score-total')).toHaveText('8');
 });
 await run('purchase with empty scores permits negative net total and reset restores token',async p=>{
  await p.selectOption('#roll','3');p.once('dialog',d=>d.accept());await p.click('#random-remaining');await expect(p.locator('#score-total')).toHaveText('-10');await p.click('#reset');await expect(p.locator('#score-total')).toHaveText('0');await expect(p.locator('#extra')).toBeChecked();
 });
