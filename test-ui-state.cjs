...
(async()=>{
 ...
 for(const width of [1440,390])await run(`loader and stale result race ${width}`,async page=>{
  ...
  await page.fill('#score-0','10');await expect(page.locator('#freshness')).toHaveText('Nieaktualne');
  const originalResults=await page.locator('#results').innerText();
  ...
  await page.fill('#score-0','50');await expect(page.locator('#freshness')).toHaveText('Nieaktualne');
  ...
  assert.equal(await page.locator('#results').innerText(),originalResults);
  await page.screenshot({path:`test-artifacts/state-${width}-stale.png`,fullPage:true});
  await calculate(page);await expect(page.locator('#freshness')).toHaveText('Aktualne');const ev=Number((await page.locator('.ev-number').first().innerText()).replace(/\s|pkt/g,'').replace(',','.'));await expect(page.locator('.end-score b').first()).toHaveText((50+ev).toLocaleString('pl-PL',{minimumFractionDigits:3,maximumFractionDigits:3})+' pkt');
 ...
 await run('every input invalidates until explicit recalculation',async page=>{
  await ready(page);
  const changes=[...Array.from({length:5},(_,i)=>()=>page.fill('#die-'+i,'3')),...Array.from({length:5},(_,i)=>()=>page.check('#lock-'+i)),...Array.from({length:7},(_,i)=>()=>page.check('#used-'+i)),()=>page.selectOption('#roll','2'),()=>page.uncheck('#extra'),()=>page.fill('#score-0','17'),()=>page.click('#reset')];
