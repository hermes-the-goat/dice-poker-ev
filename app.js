...
function reconcileScores(){
 ...
 $('score-total').textContent=String(netTotal());$('score-error').textContent='';return true;
}
...
function randomize(all){
 ...
 if(!all&&s.roll===3){
  if(!window.confirm('Kupić czwarty rzut za 10 pkt? Wynik netto zmniejszy się o 10 pkt. Dodatkowy rzut jest dostępny tylko raz na grę.'))return;
  $('extra').checked=false;
 }
 initial.forEach((_,i)=>{
...
function render(all,{s,total,occupied,revision:resultRevision}){
  ...
  top.forEach((a,i)=>{
   ...
   hint.textContent=recordable?'Do tabeli trafią punkty figury, nie EV.':'Najpierw wykonaj rzut';
...
$('reset').addEventListener('click',()=>{initial.forEach((v,i)=>{$('die-'+i).value=v;$('lock-'+i).checked=false;});names.forEach((_,c)=>{$('used-'+c).checked=false;$('score-'+c).value='';scores[c]=null;});$('roll').value='1';$('extra').checked=true;reconcileScores();invalidate();});
